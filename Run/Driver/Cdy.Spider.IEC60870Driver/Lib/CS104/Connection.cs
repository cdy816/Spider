...
namespace lib60870.CS104
{
    ...
    /// </summary>
    public class NetConnection : Master
    {
        ...
        private int receiveTimeoutInMs = 2000; /* maximum allowed time between SOF byte and last message byte */
        ...
        private bool SendNextWaitingASDU()
        {
            ...
            {
                LoggerService.Service.Info("CS104", "connection lost");
                ////running = false;
                //throw new ConnectionException("connection lost");
                return false;
            }
        ...
        private void SendASDUInternal(ASDU asdu)
        {
            lock (socket)
            {
                ...
                else
                {
                    ...
                    if (IsSentBufferFull())
                    {
                        LoggerService.Service.Info("CS104", "Flow control congestion. Try again later.");
                        return;
                    }
        ...
        private void Setup(APCIParameters apciParameters, ApplicationLayerParameters alParameters)
        {
        ...
        public NetConnection(ICommChannel2 msocket)
        {
            this.socket = msocket;
            Setup(new APCIParameters(), new ApplicationLayerParameters());
        }
        ...
        /// </summary>
        /// <param name="msocket"></param>
        /// <param name="apciParameters"></param>
        /// <param name="alParameters"></param>
        public NetConnection(ICommChannel2 msocket,APCIParameters apciParameters, ApplicationLayerParameters alParameters)
        {
            this.socket = msocket;
            Setup(apciParameters.Clone(), alParameters.Clone());
        }
        ...
        public NetConnection(ICommChannel2 msocket, APCIParameters apciParameters, ApplicationLayerParameters alParameters, TlsSecurityInformation tlsInfo)
        {
            this.socket = msocket;
            Setup(apciParameters.Clone(), alParameters.Clone());
            tlsSecInfo = tlsInfo;
        ...
        public void SendStartDT()
        {
            if (running)
            {
                try
                {
                    ...
                    socket.Send(STARTDT_ACT_MSG, 0, STARTDT_ACT_MSG.Length);
                }
                catch (Exception ex)
                {
        ...
        public void SendStopDT()
        {
            if (running)
            {
                try
                {
                    ...
                    socket.Send(STOPDT_ACT_MSG, 0, STOPDT_ACT_MSG.Length);
                }
                catch (Exception ex)
                {
                    throw new ConnectionException("Failed to write to socket", ex);
        ...
        protected void SendStartDT_CON()
        {
            if (running)
            {
                try
                {
                    socket.Send(STARTDT_CON_MSG, 0, STARTDT_CON_MSG.Length);
                }
                catch (Exception ex)
                {
                    throw new ConnectionException("Failed to write to socket", ex);
        ...
        protected void SendStopDT_CON()
        {
            if (running)
            {
                try
                {
                    socket.Send(STOPDT_CON_MSG, 0, STOPDT_CON_MSG.Length);
                }
                catch (Exception ex)
                {
                    throw new ConnectionException("Failed to write to socket", ex);
        ...
        protected void SendTestFR_ACT()
        {
            if (running)
            {
                try
                {
                    socket.Send(TESTFR_ACT_MSG, 0, TESTFR_ACT_MSG.Length);
                }
                catch (Exception ex)
                {
                    throw new ConnectionException("Failed to write to socket", ex);
        ...
        protected void SendTestFR_CON()
        {
            if (running)
            {
                try
                {
                    socket.Send(TESTFR_CON_MSG, 0, TESTFR_CON_MSG.Length);
                }
                catch (Exception ex)
                {
                    throw new ConnectionException("Failed to write to socket", ex);
        ...
        private int receiveMessage(byte[] buffer)
        {
            ...
            //if (socket.Poll(50, SelectMode.SelectRead))
            {
                ...
                if (readState == 0)
                {
                    ...
                    var len = socket.Read(buffer, 0, 1,this.receiveTimeoutInMs);
                    if (len != 1) return -1;
                ...
                {
                    // read length byte
                    if (socket.Read(buffer, 1, 1, this.receiveTimeoutInMs) != 1)
                        return 0;
                ...
                if (readState == 2)
                {
                    int readLength = socket.Read(buffer, currentReadPos, remainingReadLength, this.receiveTimeoutInMs);
        ...
        public void ProcessReceive()
        {
            ...
            while (running)
            {
                ...
                {
                    LoggerService.Service.Info("CS104", se.StackTrace);
                    continue;
                }
                catch (System.IO.IOException e)
                {
                    LoggerService.Service.Info("CS104", e.StackTrace);
                    continue;
                }
                catch (ConnectionException ee)
                {
                    LoggerService.Service.Info("CS104", ee.StackTrace);
                    continue;
                }
            }
        }
